  ...
  if (tolower (hdr[0]) == 's' && line->iov_len > 12
      ...
      if (len + 10 >= MAX_COOKIE_LEN)
	{
      ...
      *end = 0;
      if (priv->cookie_len == 0) {
	      int len;
	      len = snprintf(priv->cookie, MAX_COOKIE_LEN-1, 
			     "Cookie: %s\r\n",
			     start);
	      priv->cookie[len] = 0;
	      priv->cookie_len = len;
	} else if (priv->cookie_len < MAX_COOKIE_LEN-1 &&
                   strstr(start,"_VYU_VT") == NULL) {
	      int len = priv->cookie_len - 2; 
	      len += snprintf(priv->cookie + len, MAX_COOKIE_LEN - 1 - len, 
			     "; %s\r\n",
			      start);
	      priv->cookie[len] = 0;
	      priv->cookie_len = len;
